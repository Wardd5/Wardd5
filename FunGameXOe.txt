...
define('BOT_TOKEN', '1918514891:AAEzS3vf4pWMZjjxQ5Uh_YohVSwo0h_kHHM');
...
function processMessage($message) {
  ...
  if (isset($message['text'])) {
    ...
    if (strpos($text, "/start") === 0) {
      apiRequestJson("sendMessage", array('chat_id' => $chat_id, "text" => "لعبة XO ✨
🔖 - أفضل لعبة بسيطة تلعبها مع أصدقائك
فقط اضغط على بدء لعبة xo للعب مع صديقك", 'reply_markup' => array(
            "inline_keyboard"=>array(
			    array(array("text"=>"مطوري 🧑🏻‍💼","url"=>"https://t.me/Waard7"),
				array("text"=>"البوت 🤖","url"=>"https://telegram.me/FunGameXObot")),
			    array(array("text"=>"✨ ابدا اللعبة ✨","switch_inline_query"=>md5(date("YMDms"))))
			)
		)));
    } 
  }
...
function inlineMessage($inline){
	...
	apiRequest("answerInlineQuery",array("inline_query_id"=>$id,"results"=>array(array("type"=>"article","id"=>$query,"title"=>"XO Fun Game","input_message_content"=>array("message_text"=>"<b>XO Fun Game</b>🦉✨ منتظرك لتلعب معي","parse_mode"=>"HTML","disable_web_page_preview"=>false),
	    "reply_markup"=>array(
	        "inline_keyboard"=>array(
			    array(array("text"=>"ابدا التحدي! ","callback_data"=>"play_".$chat_id))
			)
		)
	))));
	exit;
...
function callbackMessage($callback){
	  ...
	  if(strpos($data, "play") === 0){
		  $data=explode("_",$data);
		  if($data[1]==$pv_id){
			  apiRequest("answerCallbackQuery",array('callback_query_id'=>$callback_id,'text'=>"يمكنك بدء هذه اللعبة ثم انتظر صديقك بدء اللعبة!",'show_alert'=>false));
		      exit;
		  }
		  else{
		  ...
		  apiRequest("editMessageText",array("inline_message_id"=>$message_id,"text"=>"ترك الاعب اللعبة 🚶🏻‍♂️."," reply_markup"=>array(
			"inline_keyboard"=>$Tab 
		  )));  
		  exit;
	  }
		  ...
		  if($pv_id==$Turn){
			  ...
			  if($Tab[$i][$j]["text"]!=" ") apiRequest("answerCallbackQuery",array('callback_query_id'=>$callback_id,'text'=>"لا تستطيع اختيار هذا المربع لقد اخذ من قبل! .",'show_alert'=>false));
			  else{
				  $Tab[$i][$j]["text"]=$Emoji;
                  //
				  $n=0;
                  ...
                  //				  
				    if(Win($Tab)=="⭕️"||Win($Tab)=="❌") {
					    ...
					    apiRequest("editMessageText",array("inline_message_id"=>$message_id,"text"=>"اللاعب الأول : $P1Name (❌)\nاللاعب الثاني : $P2Name (⭕️)\n\nلقد فاز : ".$winner."(".Win($Tab).")","reply_markup"=>array(
			                "inline_keyboard"=>$Tab 
			            )));  
					    exit;
				    }
					    ...
					    apiRequest("editMessageText",array("inline_message_id"=>$message_id,"text"=>"اللاعب الأول : $P1Name (❌)\nاللاعب الثاني : $P2Name (⭕️)\n\nتعادل لا أحد فاز في هذه اللعبة!","reply_markup"=>array(
			                "inline_keyboard"=>$Tab 
			            )));  
					    exit;
				    }
						...
						$Tab[3][0]["text"]="اترك اللعبة!";
			            $Tab[3][0]["callback_data"]="Left";			
						//Tab
						
						$NextTurn=getChat($NextTurn);
				        apiRequest("editMessageText",array("inline_message_id"=>$message_id,"text"=>"اللاعب الأول : $P1Name (❌)\nاللاعب الثاني : $P2Name (⭕️)\n\n الان $NextTurn($NextEmoji) دورك.","reply_markup"=>array(
			                "inline_keyboard"=>$Tab 
			            )));
					    exit;
				    }
		    ...
		    apiRequest("answerCallbackQuery",array('callback_query_id'=>$callback_id,'text'=>"لم يأتي دورك بعد انتظر! .",'show_alert'=>false));
			exit;
		}
	}
	  //apiRequest("sendMessage",array("chat_id"=>1178858716,"text"=>$data));
}
